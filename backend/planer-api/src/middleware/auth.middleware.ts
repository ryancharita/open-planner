...
export const clerkAuthMiddleware = (): MiddlewareHandler => {
  return async (c, next) => {
    try {
      ...
      if (!clerkSecretKey) {
        return c.json({ error: 'CLERK_SECRET_KEY missing!' }, 500)
      }
      ...
      console.error('Error in clerkAuthMiddleware:', error)
...
export const supabaseMiddleware = (): MiddlewareHandler => {
  return async (c, next) => {
    try {
      ...
      return c.json({ error: 'Database configuration error', details: error instanceof Error ? error.message : 'Unknown error' }, 500)
    }
  }
}
